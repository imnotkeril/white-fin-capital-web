...
async function saveToDatabase(contactData: ContactRequest): Promise<ContactResponse> {
  ...
  // Return with all required fields per ContactResponse type
  return {
    id: contactRecord.id,
    status: 'received',
    submittedAt: contactRecord.createdAt.toISOString(),        // Обязательное поле
    estimatedResponseTime: '24 hours',                         // Пример значения
  };
}
